...
void compchem::Molecule::computeSCF(bool print) {
	...
	do {
		...
		if (first) {
			...
			cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans, this->orbitals,
					this->orbitals, this->orbitals, 1,
					this->orthogonal->getArray(), this->orbitals,
					this->hamiltonian->getArray(), this->orbitals, 0,
					this->orthogonal_eigvs->getArray(), this->orbitals);
			...
			if (print) {
				printArray("Overlap", this->overlap);
				printArray("Kinetic Energy", this->kinetic);
				printArray("Nuclear Attraction", this->attraction);
				printArray("Hamiltonian", this->hamiltonian);
				printArray("Orthogonalization", this->orthogonal);
				printArray("Initial F' Matrix",
						new Array<double>(temp, 2, this->orbitals,
								this->orbitals));
			}
			...
			cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans, this->orbitals,
					this->orbitals, this->orbitals, 1,
					this->orthogonal->getArray(), this->orbitals,
					this->fock->getArray(), this->orbitals, 0,
					this->orthogonal_eigvs->getArray(), this->orbitals);
		...
		qsort(this->molecular_energies->getArray(), this->orbitals, sizeof(double), compared);

		//Sift through data.
		for (int i = 0; i < this->orbitals; i++) {
			unsigned long off = ((unsigned long) adds[i]
					- (unsigned long) this->molecular_energies->getArray());
			off /= sizeof(double);
			for (int j = 0; j < this->orbitals; j++) {
		...
		cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, this->orbitals,
				...
				this->orbitals, this->molecular_orbitals->getArray(),
				this->orbitals, 0, this->orthogonal_t->getArray(),
				this->orbitals);
		if (print && first) {
			printArray("Initial C", this->orthogonal_t);
		}

		memmove(this->molecular_orbitals->getArray(), this->orthogonal_t->getArray(), whole_mat);
		...
		if (print && first) {
			printArray("Initial Density", this->density);
		}
		...
		if (first && print) {
			printArray("Second Fock matrix", this->fock);
	...
	if (print) {
		printArray("Density", this->density);
	}

	for(int i = 0; i < this->orbitals; i++) {
		(*(this->molecular_energies))(i, i) = (*(this->molecular_energies))(0, i);
		if(i != 0) {
