...
void compchem::printMoments(Molecule *molecule) {
	puts("\nCenter of Mass");
	printf("%f %f %f\n", molecule->getCenterOfMass()[0], molecule->getCenterOfMass()[1], molecule->getCenterOfMass()[2]);

	puts("\nMoments of Inertia");
	for (int i = 0; i < 3; i++) {
		printf("%5.5f %5.5f %5.5f\n", molecule->getInertialMoments()(i, 0),
				molecule->getInertialMoments()(i, 1), molecule->getInertialMoments()(i, 2));
	}

	puts("\nPrimary moments (amu bohr^2)");
	printf("%5.5f %5.5f %5.5f\n", molecule->getPrincipleMoments()(0), molecule->getPrincipleMoments()(1), molecule->getPrincipleMoments()(2));
	puts("\nPrimary moments (amu \u00e5^2)");
	printf("%5.5f %5.5f %5.5f\n", molecule->getPrincipleMoments()(0) * .2800285, molecule->getPrincipleMoments()(1) * .2800285,
			molecule->getPrincipleMoments()(2) * .2800285);
	puts("\nPrimary moments (g cm^2)");
	printf("%5.5e %5.5e %5.5e\n", 1.660468e-24 * molecule->getPrincipleMoments()(0) * .00000000000000002800285,
			1.660468e-24 * molecule->getPrincipleMoments()(1) * .00000000000000002800285,
			1.660468e-24 * molecule->getPrincipleMoments()(2) * .00000000000000002800285);

	puts("\nRotational Constants (1/cm)");
	printf("%f %f %f\n", molecule->getRotationalConstants()(0), molecule->getRotationalConstants()(1),
			molecule->getRotationalConstants()(2));

	puts("\nRotational Constants (MHz)");
	printf("%f %f %f\n", 29979.2458 * molecule->getRotationalConstants()(0), 29979.2458 * molecule->getRotationalConstants()(1),
...
void compchem::printFreqs(Molecule *molecule) {
	puts("\nHarmonics.");
	for(int i = molecule->getHessianSize() - 1; i >= 0; i--) {
		printf("%d\t%f\n", i, sqrt((*(molecule->getHessianEigs()))(i)) * 5140.697669352);
	}
	fflush(stdout);
}
...
void compchem::printSCF(Molecule *molecule) {
	...
	for(int i = 0; i < molecule->getNumatoms(); i++) {
		printf("Charge on atom %d: %f\n", i, molecule->getAtoms()[i].getTrueCharge());
	}
	fflush(stdout);
}

void compchem::printArray(char *title, Array<double> *array) {
	puts("");
	puts(title);
